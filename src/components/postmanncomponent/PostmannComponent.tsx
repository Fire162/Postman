...
import {
  atomDark,
  ghcolors,
} from "react-syntax-highlighter/dist/esm/styles/prism"; // Import the new theme
import {
  Github,
  HeartFill,
  TextWrap,
...
const PostmannComponent: React.FC<PostmannComponentProps> = () => {
  ...
  const [url, setUrl] = useState<string>(
    localStorage.getItem("postmannUrl") || ""
  ); // Load from localStorage
  const [jsonBody, setJsonBody] = useState<string>(
    localStorage.getItem("postmannJsonBody") || ""
  ); // Load from localStorage
  ...
  const [viewOption, setViewOption] = useState<
    "pretty" | "raw" | "image" | "binary" | "pdf" | "headers"
  >("pretty");
  const [responseTime, setResponseTime] = useState<number | null>(null);
  const [loading, setLoading] = useState<boolean>(false);
  const [syntaxHighlighterLanguage, setSyntaxHighlighterLanguage] =
  ...
  const [isModifyRequestHeadersModalOpen, setIsModifyRequestHeadersModalOpen] =
  ...
  const CopyButton: React.FC<CopyButtonProps> = ({ contentToCopy }) => {
    ...
    return (
      <button
        onClick={() => {
          handleCopy();
          handleButtonClick(copyButtonRef);
        }}
        className={`copy-button ${copied ? "flash" : ""}`}
        ref={copyButtonRef}
      >
        <Clipboard className="rbi-button-icon" />
        {copied ? "Copied" : "Copy"}
      </button>
    );
  };
  ...
  const optionsDiv = (type: string, resp: any) => {
    let fileName =
      type === "pretty" ? "response-body.txt" : "response-headers.txt";
    ...
    return (
      <div className="optionsDiv">
        ...
        <button
          onClick={() => {
            toggleLineNumbers();
            handleButtonClick(toggleLnumButtonRef);
          }}
        ...
        <button
          onClick={() => {
            ...
            handleButtonClick(wrapButtonRef);
          }}
          className="wrap-button"
          ref={wrapButtonRef}
        >
          <TextWrap className="rbi-button-icon" />{" "}
          {wrapState ? "!!Wrap" : "Wrap"}
        </button>
        ...
        <button
          onClick={() => {
            handleDownload();
            handleButtonClick(downloadButtonRef);
          }}
          className="download-button"
          ref={downloadButtonRef}
        >
          <Download className="rbi-button-icon" />
          Download
        </button>
        ...
        <button
          onClick={() => {
            setThemeForSHL(themeForSHL == atomDark ? ghcolors : atomDark);
            handleButtonClick(themeButtonRef);
          }}
          className="theme-button"
          ref={themeButtonRef}
        >
          {themeForSHL == atomDark ? (
            <BrightnessHighFill className="rbi-button-icon" />
          ) : (
            <MoonFill className="rbi-button-icon" />
          )}
    ...
    
{
  ...
  const replaceVariables = (input: string, typeOfInput: string) => {
    ...
    const variables: Variable[] = storedVariables
      ? JSON.parse(storedVariables)
      : [];
  ...
  const sendRequest = async () => {
    ...
    try {
      ...
      let savedHeadersArray: HeaderObject[] = JSON.parse(
        savedHeadersJSON || "[]"
      );

      // Convert the array of HeaderObject into a single object
      let savedHeadersJSONObj: Record<string, string> =
        savedHeadersArray.reduce(
          (accumulator: Record<string, string>, current: HeaderObject) => {
  ...
  const renderDonationLink = () => {
    if (country === "IN") {
      ...
      return (
        <a
          href="https://www.paypal.com/paypalme/shatadip2020"
          target="_blank"
  ...
  return (
    <div className={`postmann-container`}>
      <div className="postmann-header">
        <img
          src="postmann-icon128.png"
          alt="Postmann Logo"
          className="postmann-logo rotating"
        />
        ...
        <div className="postmann-links">
          {renderDonationLink()}
          <a
            href="https://github.com/shatadip/Postmann"
            target="_blank"
            rel="noopener noreferrer"
          >
      ...
      <div className="dynamic-options mt-equal-1">
        <a href="#" onClick={renderVariablesModal}>
          Variables
        </a>
        <a href="#" onClick={renderHistoryModal}>
          History
        </a>
        <a href="#" onClick={renderNotesModal}>
          Notes
        </a>
        <a href="#" onClick={renderMRHModal}>
        ...
        {requestSent && ( // Render the response section only if the request has been sent
          <>
            <p className={`postmann-response-title`}>
              Response:
              <span
                className={`prt-code ${responseClass}`}
                data-tooltip="Status Code"
              >
                <strong>
                  {responseCode !== null ? (
                    responseCode
                    ...
                    <>
                      <i className="hourGlassEmoji-styles">⌛</i>
                    </>
                  )}
                </strong>
              </span>
              <span className="prt-time" data-tooltip="Response Time">
                {responseTime !== null
              ...
              {responseCode &&
                ...
                responseHeaders && (
                  <span
                    className="prt-size"
                    data-tooltip={`Response Size: ${calculateSizeInBytes(
                      absoluteRawResponse
                    )} (body) + ${calculateSizeInBytes(
                      formatHeaders2(responseHeaders)
                    )} (headers)`}
                  >
                    {calculateSizeInBytes2(
                      absoluteRawResponse,
                      formatHeaders2(responseHeaders)
                    )}
                  </span>
            ...
            <div className="postmann-tabs">
              {responseType === "image" ? (
                <div
                  className={`postmann-tab ${
                    viewOption === "image" ? "active" : ""
                  }`}
                ...
                <>
                  <div
                    className={`postmann-tab ${
                      viewOption === "pretty" ? "active" : ""
                    }`}
                    onClick={() => setViewOption("pretty")}
                  >
                    Pretty
                    {isInvalidJSON && (
                      <sup className="invalidJSON-sup">Invalid JSON</sup>
                    )}
                  </div>
                  <div
                    className={`postmann-tab ${
                      viewOption === "raw" ? "active" : ""
                    }`}
                    onClick={() => setViewOption("raw")}
                  >
                    Raw
                  </div>
                </>
              ...
              <div
                className={`postmann-tab ${
                  viewOption === "headers" ? "active" : ""
                }`}
                onClick={() => setViewOption("headers")}
              >
                Headers
              </div>
            </div>
      ...
      <Modal
        ...
        className={`donation-modal ${
          isClosing ? "ReactModal__Content--before-close" : ""
        }`}
      ...
      <Modal
        ...
        onRequestClose={closeModal}
        contentLabel="Donation Modal"
        className={`donation-modal vars-modal ${
          isClosing ? "ReactModal__Content--before-close" : ""
        }`}
        overlayClassName="donation-modal-overlay"
        shouldCloseOnOverlayClick={true}
      ...
      <Modal
        ...
        onRequestClose={closeModal}
        contentLabel="Donation Modal"
        className={`donation-modal history-modal ${
          isClosing ? "ReactModal__Content--before-close" : ""
        }`}
        overlayClassName="donation-modal-overlay"
        shouldCloseOnOverlayClick={true}
      ...
      <Modal
        ...
        onRequestClose={closeModal}
        contentLabel="Donation Modal"
        className={`donation-modal notes-modal ${
          isClosing ? "ReactModal__Content--before-close" : ""
        }`}
        overlayClassName="donation-modal-overlay"
        shouldCloseOnOverlayClick={true}
      ...
      <Modal
        ...
        onRequestClose={closeModal}
        contentLabel="Donation Modal"
        className={`donation-modal mrh-modal ${
          isClosing ? "ReactModal__Content--before-close" : ""
        }`}
        overlayClassName="donation-modal-overlay"
        shouldCloseOnOverlayClick={true}
      ...
      <Modal
        ...
        onRequestClose={closeModal}
        contentLabel="Donation Modal"
        className={`donation-modal about-modal ${
          isClosing ? "ReactModal__Content--before-close" : ""
        }`}
        overlayClassName="donation-modal-overlay"
        shouldCloseOnOverlayClick={true}
